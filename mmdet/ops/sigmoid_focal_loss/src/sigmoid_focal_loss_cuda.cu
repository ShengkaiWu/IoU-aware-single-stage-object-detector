...
__global__ void SigmoidFocalLossForward(const int nthreads,
  ...
  CUDA_1D_KERNEL_LOOP(i, nthreads) {
    ...
    // log(p): don't compute focal loss for positive examples
    scalar_t term1_non_focal = logf(max(p, FLT_MIN));
...
__global__ void SigmoidFocalLossBackward(
  ...
  CUDA_1D_KERNEL_LOOP(i, nthreads) {
    ...
    // (1-p)**g * (1 - p - g*p*log(p)): this is the derivative to logits[i]
    scalar_t term1 =
        powf((1. - p), gamma) * (1. - p - (p * gamma * logf(max(p, FLT_MIN))));

    // added by Shengkai Wu
    // compute the derivative of log(p) relative to logits[i];
    scalar_t term1_non_focal = (1. - p);
